...
def SVM_model_train(data_train, data_test, data_train_label, data_test_label):
    # set random forest
    model = OneVsRestClassifier(svm.SVC(kernel='rbf'))
    model = model.fit(data_train,data_train_label)
    return model


def SVM_model_predict(model,sample_2010_norm,im_data_land2010,im_data_land2015,im_height,im_width,number,output_path):
